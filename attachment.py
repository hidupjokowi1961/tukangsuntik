...
from email.utils import formataddr  # 🔥 untuk gabung sender name + email
...
def load_configs():
    files = sorted([f for f in os.listdir() if f.endswith(".json")])  # 🔥 urutkan sesuai abjad
    configs = []
    for f in files:
        try:
    ...
    # 🔥 urutkan lagi berdasarkan nama config kalau ada "name"
    configs.sort(key=lambda x: x.get("name", x["_filename"]).lower())
    return configs
...
def tampilkan_detail(config):
    ...
    sections = [
        (["name", "from_name", "smtp_server", "port"], 12),   # 🔥 tampilkan from_name juga
        (["email_user", "email_pass", "to"], 12),
        (["subject", "body"], 12),
        (["attachment"], 12)  # 🔥 tampilkan attachment juga jika ada
    ]
...
def kirim_email(config, count, delay):
    try:
        ...
        # 🔥 atur alamat From
        if "from_name" in config and config["from_name"].strip():
            from_addr = formataddr((config["from_name"], config["email_user"]))
        else:
        ...
        for i in range(1, count+1):
            ...
            if "attachment" in config and config["attachment"].strip():
                filepath = config["attachment"]
                if os.path.exists(filepath):
                    ...
                    print(f"⚠️ File attachment tidak ditemukan: {filepath}")
